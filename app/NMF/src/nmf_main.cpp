...
void Run() {
	...
	for (int i = 0; i!=1000; ++i) {
		// pick a random row and read it
		int row_id = rand() % FLAGS_number_rows;
		petuum::RowAccessor row_acc;
		const auto& row = table.Get<petuum::DenseRow<float>> (row_id, &row_acc);
		std::vector<float> vals	 (FLAGS_row_size);
		row.CopyToVector(&vals);
